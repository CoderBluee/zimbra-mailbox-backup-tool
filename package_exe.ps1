...
# Description: Automated end-to-end pipeline to compile Prism Zimbra Mailbox Backup Tool Tool,
...
param (
    ...
    [switch]$AppImageOnly      # Pass -AppImageOnly to stop after creating Prism-Zimbra-Mailbox-Backup-Tool.exe
)
...
$AppFolder = Join-Path $AppImageDir "Prism-Zimbra-Mailbox-Backup-Tool"
...
Write-Host "  Prism Zimbra Mailbox Backup Tool Tool - Full EXE & Installer Pipeline" -ForegroundColor Cyan
...
$JpackageArgs = @(
    "--type", "app-image",
    "--name", "Prism-Zimbra-Mailbox-Backup-Tool",
...
$ExePath = Join-Path $AppFolder "Prism-Zimbra-Mailbox-Backup-Tool.exe"
if (-not (Test-Path $ExePath)) {
    Write-Error "Prism-Zimbra-Mailbox-Backup-Tool.exe was not created at expected location: $ExePath"
}
...
& $AiPath /edit $AipFile /SetProperty ProductName="Prism Zimbra Mailbox Backup Tool Tool"
...
& $AiPath /edit $AipFile /NewShortcut -name "Prism Zimbra Mailbox Backup Tool Tool" -dir DesktopFolder -target "[APPDIR]Prism-Zimbra-Mailbox-Backup-Tool\Prism-Zimbra-Mailbox-Backup-Tool.exe" -icon "$LogoIcon"
& $AiPath /edit $AipFile /NewShortcut -name "Prism Zimbra Mailbox Backup Tool Tool" -dir SHORTCUTDIR -target "[APPDIR]Prism-Zimbra-Mailbox-Backup-Tool\Prism-Zimbra-Mailbox-Backup-Tool.exe" -icon "$LogoIcon"
...
& $AiPath /edit $AipFile /SetPackageName "Prism-Zimbra-Mailbox-Backup-Tool-Setup.exe" -buildname DefaultBuild
...
# 10. Sync to Drive
$driveJarDir = "I:\My Drive\Akas\Jars\Zimbra Mailbox Backup Tool"
if (-not (Test-Path $driveJarDir)) { New-Item -ItemType Directory -Path $driveJarDir -Force | Out-Null }
Copy-Item $ProtectedJar.FullName (Join-Path $driveJarDir "Prism-Zimbra-Mailbox-Backup-Tool.jar") -Force
...
Copy-Item $AipFile (Join-Path $destAdvance "Prism-Zimbra-Mailbox-Backup-Tool.aip") -Force
...
Write-Host "     -> Prism-Zimbra-Mailbox-Backup-Tool-Setup.exe" -ForegroundColor White
Write-Host "  3. Standalone Protected JAR: $driveJarDir" -ForegroundColor White
Write-Host "     -> Prism-Zimbra-Mailbox-Backup-Tool.jar" -ForegroundColor White
Write-Host "========================================================" -ForegroundColor Green
